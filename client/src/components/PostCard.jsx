  ...
  return (
    <div className="bg-[#1A1A1A] rounded-[2rem] overflow-hidden animate-fade-in mb-6 border border-white/5">
      ...
      <div className="flex items-start gap-3 p-4 pb-2">
        {post.avatar ? (
          <img src={getMediaUrl(post.avatar)} alt="" className="w-10 h-10 rounded-full object-cover" />
        ) : (
            ...
            <div key={m.id} className="w-full">
              {m.media_type === 'image' && (
                <img
                  ...
                  onClick={() => window.open(getMediaUrl(m.file_path), '_blank')}
                />
              )}
              {m.media_type === 'video' && (
                <video
              ...
              {m.media_type === 'audio' && (
                <div className="flex items-center gap-3 p-4 bg-[#2A2A2A] mx-4 mb-3 rounded-xl">
                  ...
                  <audio src={getMediaUrl(m.file_path)} controls className="flex-1 h-10" />
                </div>
              )}
            </div>
          ))}
